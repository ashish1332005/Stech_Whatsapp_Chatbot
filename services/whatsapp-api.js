 ...
 */
const saveOutgoingMessage = async (waId, text, isAgentReply) => {
    try {
        const newMessage = new Message({
            waId: waId,
            content: text,
            direction: 'outbound',
            chatStatus: isAgentReply ? 'live_agent_mode' : 'bot_mode',
        });
        await newMessage.save();
    } catch (error) {
        console.error(`[DB] Error saving outgoing message to ${waId}:`, error);
    }
};
export const saveIncomingMessage = async (waId, messageData, metaId, chatStatus) => {
    try {
        const newMessage = new Message({
            waId: waId,
            content: messageData.text?.body || `[${messageData.type.toUpperCase()}]`, // Text ya media type save karein
            direction: 'inbound',
            metaId: metaId,
            chatStatus: chatStatus || 'bot_mode', // Status jo index.js se aayega
        });
        await newMessage.save();
    } catch (error) {
        if (error.code === 11000) { 
            console.warn(`[DB] Duplicate message ignored for WAID: ${metaId}`);
        } else {
            console.error(`[DB] Error saving incoming message from ${waId}:`, error);
        }
    }
};
export async function sendTextMessage(to, text, isAgentReply = false) {
    try {
